...
class GenericStreamingProxy(
    ...
    override fun serve(session: IHTTPSession): Response {
        return try {
            ...
            println("PANELS: PLUGINS/com.pedropathing.panels.LIMELIGHTPROXY: Proxy error: ${e.message}")
            e.printStackTrace()
    ...
    private fun handleReverseProxy(session: IHTTPSession): Response {
        println("PANELS: PLUGINS/com.pedropathing.panels.LIMELIGHTPROXY: Proxying request to ${session.uri}")
    ...
    fun startServer() {
        if(isAlive) return
        start()
        println("PANELS: PLUGINS/com.pedropathing.panels.LIMELIGHTPROXY: Server started on port $outsidePort")
    }

    fun stopServer() {
        if(!isAlive) return
        stop()
        println("PANELS: PLUGINS/com.pedropathing.panels.LIMELIGHTPROXY: Server stopped")
    }
}