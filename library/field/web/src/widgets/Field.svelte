<script lang="ts">
    ...
    import {CanvasRotation, DrawablesTypes, emptyPacket, type Packet} from "../types"
    ...
    async function render() {
        console.log("images rendering full")

        init(canvas, new Distance(packet.offsetX), new Distance(packet.offsetY), packet.rotation)
        if (packet.bgID != null) {
    ...
    onMount(async () => {
        manager.state.onChange(manager.PACKETS_KEY, async (newValue) => {
            ...
            // packet.offsetX = 24 * -3
            // packet.offsetY = 24 * 3
            // packet.rotation = CanvasRotation.DEG_270
            await render()
        })
